...
class Threshold_Tuner(wx.Frame):
    ...
    def InitUI(self):
        ...
        self.SetMinSize((1250, 945))
        self.CreateStatusBar()
        self.SetSizer(whole_window)

        self.SetSize((1250, 945))
    ...
    def camera_monitoring_linux(self, parent):
        ...
        while(True):
            ...
            #img = wx.Image(io.BytesIO(image), type =wx.BITMAP_TYPE_ANY )
            #img = wx.Image(975, 601, image )
            print('test0')
            img = wx.Bitmap.FromBuffer(480, 360, cv2_image )
            """
            height1, width1 = cv2_image.shape[:2]
            print('test1')
            img = wx.Image(width1, height1)
            img.SetData(cv2_image.tobytes())
            #img = image.ConvertToBitmap()
            print('test3')
            self.parent.bitmap1 = wx.Bitmap(img.Scale(480,360))
            """
            self.parent.bitmap2 = self.parent.bitmap1 = img
            self.parent.Refresh()
            if not self.connction_stop_event.is_set(): 
                try:
                    raise ValueError('oops!')

...
if __name__ == "__main__":
    try:
        ...
        while cv2.waitKey(10) != 27:
            ...
            if time.perf_counter() - timer > counter + 1:
                counter += 1
                os.system("espeak -ven-m1 -a50 " + str(counter))
            if time.perf_counter() - timer >= 5:
                timer = time.perf_counter()
                counter = 0
                filenumber += 1
                print('sample' + str(filenumber) + '.png')
                cv2.imwrite('sample' + str(filenumber) + '.png', image)
                if filenumber == 30 : break
        # Release memory
        del frame
        # print("Stop preview...")
        # camera.stop_preview()

...
class rpc:
    ...
    def stream_reader(self, call_back, queue_depth=1, read_timeout_ms=5000): # public
        pass
        #self._stream_put_bytes(self._set_packet(0xEDF6, struct.pack("<I", queue_depth)), 5000)
        try: self._stream_put_bytes(self._set_packet(0xEDF6, struct.pack("<I", queue_depth)), 5000)
...
class rpc_master(rpc):
    ...
    def call(self, name, data=bytes(), send_timeout=1000, recv_timeout=1000): # public
        hash = self._hash(name, len(name))
        put_result = self.__put_command(hash, data, send_timeout)
        if put_result:
            result = self.__get_result(recv_timeout)
        else: result = None
        return result
...
class rpc_network_master(rpc_master):
    ...
    def _flush(self): # protected
        ...
        if self.__tcp__socket is not None:
            try:
                self.__tcp__socket.settimeout(0.001)    #0.001
                while(True):

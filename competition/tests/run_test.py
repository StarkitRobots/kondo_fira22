    ...
    def run(self):
        ...
        while True:
            ...
            for cycle in range(number_Of_Cycles):
                ...
                stepLength1 = self.stepLength
                if cycle ==0 : stepLength1 = self.stepLength/3
                if cycle ==1 : stepLength1 = self.stepLength/3 * 2
                ...
                if self.motion.i_see_ball:
                    stepLength1 = self.stepLength/3 * 2
                    self.motion.refresh_Orientation()
                    rotation = 0 + invert * self.motion.body_euler_angle['yaw'] * 1.0
                    rotation = self.motion.normalize_rotation(rotation)
                    self.motion.walk_Cycle(stepLength1,sideLength, rotation, 1, 3)
                    stepLength1 = self.stepLength/3
                    self.motion.refresh_Orientation()
                    rotation = 0 + invert * self.motion.body_euler_angle['yaw'] * 1.0
                    rotation = self.motion.normalize_rotation(rotation)
                    self.motion.walk_Cycle(stepLength1,sideLength, rotation, 1, 3)
